...
// Initialize the gyro
        ...
        myExposureControl.setExposure(15, TimeUnit.MILLISECONDS);         int cameraMonitorViewId = hardwareMap.appContext.getResources().getIdentifier("cameraMonitorViewId", "id", hardwareMap.appContext.getPackageName());
        ...
        telemetry.update();
        if(position == "left"){            
            moveStraight('f',1300,0,.5);
            waiting(0.5);
            pointTurn('l',25,.4);
            waiting(0.5);
            armMove('d','l',1);
            waiting(.25);
            moveStraight('f',100,35,.25);
            waiting(0.5);
            ...
            pointTurn('l',25,.4);
            waiting(0.5);
            moveStraight('f',150,35,.5);
            waiting(0.5);
            intakeMotor.setPower(-.5);
            waiting(0.5);
            intakeMotor.setPower(0);
            waiting(.5);
            moveStraight('b',-100,45,.5);
            waiting(.5);
            pointTurn('l',80,.4);
            waiting(.5);
            armMove('d','l',1);
            ...
            pointTurn('l',25,.4);
            waiting(0.5);
            moveStraight('f',600,40,.5);
            waiting(0.5);
            intakeMotor.setPower(-.5);
            waiting(0.5);
            intakeMotor.setPower(0);
            waiting(.5);
            moveStraight('b',-600,45,.5);
            waiting(.5);
            pointTurn('l',80,.4);
            waiting(.5);
            armMove('d','l',1);
            waiting(.5);
            moveStraight('b',-2700,90,.75);
            waiting(1);
            pointTurn('r',-80,.4);
            waiting(0.5);
            armMove('d','b',.5);
 ...
 int tfodMonitorViewId = hardwareMap.appContext.getResources().getIdentifier(
    ...
    void armMove(final char ud, final char lmh, final double motorPower){
        if(ud == 'u'){
            if(lmh == 'l'){
                while(lift.getCurrentPosition()<680){
                    lift.setPower(motorPower);

                }
            }
            else if(lmh == 'm'){
                while(lift.getCurrentPosition()<950){
                lift.setPower(motorPower);
                }
            }
            else if(lmh == 'h'){
