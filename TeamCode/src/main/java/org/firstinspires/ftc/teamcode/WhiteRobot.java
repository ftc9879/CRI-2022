...
public class WhiteRobot extends OpMode {
        ...
        if (delivering){
            ...
            while (timer.time() < totalTime) {
                ...
                while (timer.time() < accelerationTime){
                    quitIntake();
                    deliveryMotor.setPower(minPower+((maxPower-minPower)/accelerationTime*timer.time()));
                }
...
/*        if (gamepad2.x){
        ...
        public void quitIntake(){
            if (gamepad2.x || gamepad1.right_bumper){
            deliveryMotor.setPower(0);
            delivering = false;
